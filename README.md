...
Genres listing page under `/genres` utilizes `? query` functionality directly in the browser url, in order url could be only source of truth, browser back button could work properly and automatic url and api state sync.
Use `useRoute()` to read and `useRouter()` to update query params, that them would then be available to reuse across whole app under `useRoute()` for example in `useGenres` composable.

My custom composable `useGenres` has internally dynamic unique key, that is used to cache response data. For initial render of `/genres`, when all genres are fetched and displayed `ssinga-genres-${query.value.page_size}` key is used, when search is used `singa-genres-${query.value.page_size}-${query.value.name}` key is used which also includes actual query string as part of the key and when single genre is fetched `singa-genres-${genreId.value}` key is used to store response data under these 3 types of unique keys in cache and can be reused from cache for 15 minutes, before data is expired and new api request is made.
